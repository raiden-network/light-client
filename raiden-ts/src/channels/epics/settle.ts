      ...
      return combineLatest([ownBP$, partnerBP$]).pipe(
        ...
        mergeMap(([part1, part2]) =>
          transact(
            ...
            deps,
            { error: ErrorCodes.CNL_SETTLE_FAILED },
          ),
        ),
...
function coopSettleChannel(
  ...
  return transact(
    ...
    'cooperativeSettle',
    [channel.id, withdrawPairToCoopSettleParams(part1), withdrawPairToCoopSettleParams(part2)],
    deps,
    { error: ErrorCodes.CNL_COOP_SETTLE_FAILED },
  ).pipe(
    retryWhile(intervalFromConfig(config$), {
      maxRetries: 3,
      onErrors: commonAndFailTxErrors,
  ...
  return action$.pipe(
    ...
    mergeMap(([action]) => {
      ...
      return transact(
        getTokenNetworkContract(tokenNetwork),
        'unlock',
        [action.payload.id, address, partner, locks],
        deps,
        { error: ErrorCodes.CNL_ONCHAIN_UNLOCK_FAILED },
      ).pipe(
        retryWhile(intervalFromConfig(config$), {
          maxRetries: 3,
          onErrors: commonAndFailTxErrors,
